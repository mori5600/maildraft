...
impl AppState {
    ...
    /// Records one variable preset application and persists the updated store.
    ///
    /// The response returns the compact preset collection so the frontend can patch only the
    /// variable preset slice without reloading unrelated data.
    pub fn record_variable_preset_usage(&self, id: &str) -> AppResult<VariablePresetResult> {
        let started_at = Instant::now();
        let result = (|| {
            let mut store = self.store.lock().map_err(|error| error.to_string())?;
            let previous = store.clone();

            if !store.mark_variable_preset_used(id, &timestamp()) {
                return Err("指定した変数値セットが見つかりませんでした。".to_string());
            }
        ...
        match result {
            Ok((variable_preset_result, snapshot_context)) => {
                self.log_event(LogEntry {
                    level: LogLevel::Info,
                    event_name: "variable_preset.apply",
                    module: "variable_presets",
                    result: "success",
                    duration_ms: Some(elapsed_millis(started_at)),
                    error_code: None,
