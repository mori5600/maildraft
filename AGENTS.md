...
- When a feature touches `drafts`, `templates`, and `memo` in the same way, do not implement the same derivation or guard logic three times and defer cleanup. Extract a shared helper or hook during the initial change unless there is a concrete domain difference.
- When adding a shared interactive UI primitive that owns keyboard handling, suggestion building, selection state, and rendering, split state/derivation logic from presentation once the component stops being trivial. Do not leave a growing state machine inside a single UI file if the feature is expected to evolve.
- When introducing frontend/backend feature contracts such as limits, normalization rules, or validation semantics, centralize them within each language and call out any intentionally duplicated cross-language rules in the same change. Prefer adding contract-focused tests when Rust and TypeScript must stay behaviorally aligned.
- Before finalizing a cross-cutting feature, do a duplication pass on the touched files. If the same small helper, normalization rule, or filter reconciliation appears in multiple modules, either extract it immediately or document why the duplication is intentionally local.
- Prefer shipping a thin reusable abstraction early over repeating a new pattern across multiple workspaces and planning to unify it later. Small shared utilities are cheaper than post-feature cleanup once tests and UI wiring have multiplied.
